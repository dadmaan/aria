...
FeatureType = Literal["raw", "reduced"]

# Backward-compatible aliases for reduced feature types
_REDUCED_ALIASES = {"tsne", "pca", "umap", "reduced"}
...
def _resolve_feature_source(
    ...
    if path.is_dir():
        ...
        if "artifacts" in json_payload:
            ...
            artifact_metadata.setdefault("artifacts", artifact_paths)
            candidate_path = artifact_paths.get("embedding_csv") or artifact_paths.get(
                "embedding_npy"
            )
        else:
...
def load_feature_dataset(
    ...
    *,
    ...
    feature_type:
            Selects whether raw numeric features or reduced (dimensionality-reduced)
            features are returned. Accepts "raw", "reduced", or method-specific
            aliases ("tsne", "pca", "umap").
    metadata_columns:
    ...
    # Normalize reduced feature type aliases to "reduced"
    if ftype in _REDUCED_ALIASES:
        ftype = "reduced"

    if ftype not in ("raw", "reduced"):
        raise ValueError(
            f"Unsupported feature type '{feature_type}'. "
            f"Use 'raw' or 'reduced' (aliases: tsne, pca, umap)."
        )
    ...
    if not dim_columns:
        raise ValueError(
            "Reduced feature artifact does not contain numeric projection columns."
        )
