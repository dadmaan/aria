...
src: https://github.com/craffel/midi-ground-truth/blob/master/plotting.py
"""
...
def plot_hist(data, bins, xlabel=None, ylabel=None, divisor=1000):
    ...
    if np.median(height_diffs) * 20 < height_diffs.max():
        ...
        highest = max(heights)
        high_bin_indices = [
            n for n in range(len(heights)) if heights[n] > highest / 2.0
        ]
...
def uniform_hist(heights, bins, ax, **kwargs):
    ax.bar(
        x=np.arange(len(bins) - 1) - 0.5, height=heights, width=1, bottom=0, **kwargs
    )
...
def pretty_hist(heights, bins, ax=None, title=None):
    ...
    uniform_hist(heights, bins, ax, fc=FC, alpha=0.7)
    ...
    bin_spacing = 1.0
    ax.set_xlim(-bin_spacing / 2.0, len(bins) - 1 - bin_spacing / 2.0)
    if title is not None:
        plt.suptitle(title, verticalalignment="top", y=0.95, size="large")
...
def split_hist(heights, bin_edges, high_bin_indices, divisor=1000):
    ...
    low_max = 1.1 * max(
        heights[n] for n in range(len(bin_edges) - 1) if n not in high_bin_indices
    )
    # Compute the height of the smallest bin in high_bin_indices
    high_min = 0.9 * min(heights[n] for n in high_bin_indices)
    ...
    # lower plot
    ax.set_yticks(
        [int(lowtick_spacing) * int((high_min + high_max) / (2 * lowtick_spacing))]
    )
    # X-axis start of clip lines (relative to [0, 1])
    start = -0.015
    # Compute proportion of x-axis covered by last high_bin_indices (+ .015)
    end = (high_bin_indices[-1] + 1) / float(len(bin_edges) - 1) + 0.015
    # Plot the lines, allowing for it to expand outside of the axis
    ax.plot([start, end], [0.0, 0.0], transform=ax.transAxes, color="k", clip_on=False)
    ax2.plot(
        [start, end], [1.0, 1.0], transform=ax2.transAxes, color="k", clip_on=False
    )
